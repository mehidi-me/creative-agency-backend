...
const uri = `mongodb+srv://${USER}:${PASSWORD}@cluster0.lh9qm.mongodb.net/${DBNAME}?retryWrites=true&w=majority`;
...
client.connect((err) => {
      ...
      userlistCollection
        ...
        .then((result) => {
          if (result.length > 0) {
            ...
            userlistCollection
              .insertOne({ email: req.body.email, role: req.body.role })
              .then((ress) => {
  ...
  app.post("/addreview", (req, res) => {
    if (req.body !== {}) {
      ...
      clientReviewCollection
        .insertOne({ uname, cname, description, image })
        .then((ress) => {
  ...
  app.post("/addservice", (req, res) => {
    if (req.body !== {} && req.files) {
      ...
      const filePath = `${__dirname}/upload-images/${fileName}`;
      ...
      file.mv(filePath, (error) => {
        if (error) {
          ...
          serviceCollection
            .insertOne({ title, description, path })
            .then((ress) => {
  ...
  app.put("/updateorder", (req, res) => {
    ...
    orderCollection.updateOne({ _id: id }, { $set: { action } }, () => {
